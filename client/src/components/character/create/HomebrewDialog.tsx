...
import type { BackgroundInfo, RaceInfo } from '#/lib/srd'
...
export type HomebrewKind = 'race' | 'background'

/**
 * Create a race or background without leaving the wizard.
 *
...
export function HomebrewDialog({
  ...
  open: boolean
  onClose: () => void
  onCreated: (entry: RaceInfo | BackgroundInfo) => void
}) {
  ...
  const entry = kind === 'race' ? race : background
  ...
  const clash =
    ...
    (kind === 'race'
      ? (stored?.races ?? [])
      : (stored?.backgrounds ?? [])
    ).some((e) => e.name.trim().toLowerCase() === name.toLowerCase())
  ...
  const create = () => {
    ...
    save.mutate(next, {
      onSuccess: (saved) => {
        ...
        const list = kind === 'race' ? normalised.races : normalised.backgrounds
          ...
          <div className="text-xs">
            {clash && (
              <span className="text-amber-600 dark:text-amber-500">
                You already have a {kind} called &ldquo;{name}&rdquo; — saving
                replaces it.
              </span>
            )}
            {save.error && (
